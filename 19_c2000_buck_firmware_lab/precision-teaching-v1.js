(() => {
  ...
  function installRemediation(layer) {
    ...
    box.innerHTML = `
      ...
      ${({physics:'physics',sensing:'unit',timing:'timing',dynamics:'model'})[layer] ? `<p><a href="../15_power_capstone/lab_sandbox.html?remediation=${({physics:'physics',sensing:'unit',timing:'timing',dynamics:'model'})[layer]}&learnFrom=core-${layer}#training-remediation">需要多一步？做相關短實驗，再換條件驗證</a></p>` : '<p>先完成下方與本題相關的補強判斷，再回到原題驗證。</p>'}
