(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.CircuitEngineeringContext=api;})(globalThis,function(){
  ...
  function resolve(input,owners){
    ...
    const number=home||shared?0:entry?.number;
    ...
    return {known,status:known?'MODEL_ONLY':'UNKNOWN',path,moduleNumber:number,title:home?'入門工作台':shared?'同一案例的模型鏡頭':entry?.title||'尚未登錄的頁面',
      ...
      boundary:home?'前五課是固定比例穩態電路；後三課為獨立修正示意，不是同一個閉環。':shared?'共用工作點，時間與頻率鏡頭的假設不同；不可把結果混成同一模型。':entry?.number===19?'教學、SIL、HIL、target image 與真板證據分層判定。此面板不讀取或授予 BOARD_PASS。':'這裡列出模型來源與範圍；精確數值及驗證仍由原頁面與 model owner 負責。',
