(function (global) {
  ...
  if (!Schema || !QuizBank || !ModelRegistry || !Evidence || !Assessment || !raw) throw new Error("Learning runtime dependencies missing");
  ...
  function saveState(state) { return Evidence.save(state); }
  ...
  const strengthName = value => value === "A" ? "A · machine verified" : value === "B" ? "B · preregistered + machine" : value === "C" ? "C · human only" : "—";
  ...
  function familyMetrics(familyId, state) { return Assessment.mastery(familyId, state, questions, Date.now()); }
  ...
  function renderHome(rootId) {
    ...
    root.innerHTML = nav("home") + '<main id="mainContent"><section class="hero"><div class="eyebrow">Predict → Test → Explain → Transfer → Retain</div><h1>工程能力訓練系統</h1><p class="lead">核心 KPI 是未見情境的首次判斷，不是完成頁數。</p></section><section class="metric-grid"><div class="metric"><span class="tag blue">Paired baseline</span><h3>' + (benchmark.baselineAccuracy == null ? "—" : benchmark.baselineAccuracy + "%") + '</h3><p>只比較同時做過 baseline 與 transfer 的 competency。</p></div><div class="metric"><span class="tag green">Paired transfer</span><h3>' + (benchmark.transferAccuracy == null ? "—" : benchmark.transferAccuracy + "%") + '</h3><p>unseen variant 的第一次作答。</p></div><div class="metric"><span class="tag amber">Δ · N=' + benchmark.pairedN + '</span><h3>' + delta + '</h3><p>同一批 competency 的 paired 差。</p></div><div class="metric"><span class="tag rose">Calibration</span><h3>' + (cal.calibrationGap == null ? "—" : (cal.calibrationGap >= 0 ? "+" : "") + cal.calibrationGap + " pp") + '</h3><p>信心 − 實際正確率；越接近 0 越好。</p></div></section><section class="notice"><strong>證據規則：</strong>Prediction 必須先 commit；machine interaction 不等於 machine verification；模型版本會跟 evidence 一起保存。</section><section class="mode-grid">' + [["beginner.html","BEGINNER","初學路線","建立單變因因果直覺。"],["labs.html","LABS","實驗任務","先預測，再操作，再驗證。"],["troubleshooting.html","DEBUG","故障速查","用最少測試排除假設。"],["progress.html","PROGRESS","學習進度","Stage 與 evidence strength 分開看。"],["quiz.html","QUIZ","診斷測驗","首次 transfer + spaced retention。"],["search.html","SEARCH","全域搜尋","搜尋能力、模型與故障。"],["glossary.html","WORDS","詞彙表","查跨主題工程名詞。"],["report.html","REPORT","工程工作單","Prediction commit → simulator → reasoning。"] .map(x => '<a class="mode" href="' + x[0] + '"><span class="tag">' + x[1] + '</span><h2>' + x[2] + '</h2><p>' + x[3] + '</p></a>').join("") + '</section><section class="section-head"><h2>完整主題</h2><p class="muted">所有正式入口共用 normalized curriculum 與 V5 evidence。</p></section><section class="lab-grid">' + modules.map(m => '<a class="lab" href="' + esc(m.entry) + '"><div class="lesson-meta"><span class="num">' + esc(m.number) + '</span><span class="tag">' + esc(m.tag) + '</span></div><h3>' + esc(m.title) + '</h3><p>' + esc(m.oneLine) + '</p><small class="muted">' + m.lessons.length + ' 課 · ' + m.labs.length + ' 實驗</small></a>').join("") + '</section></main>';
  }
  ...
  function renderTrouble(rootId) {
    ...
    if (games.length) {
      ...
      root.querySelectorAll("[data-game-cause]").forEach(button => button.addEventListener("click", () => {
        ...
        button.closest(".diagnostic-game").querySelector(".game-score").textContent=result.solved ? "Root cause 正確；效率分數 " + result.efficiency + "/100。" : "Root cause 不符目前 evidence；繼續用辨識性測試縮小假設。";
      }));
    }
  }
  ...
  function renderQuiz(rootId) {
    ...
    function render() {
      ...
      root.innerHTML=nav("quiz")+'<main id="mainContent"><section class="hero"><div class="eyebrow">Unseen first attempt</div><h1>診斷與遷移測驗</h1><p class="lead">答錯的 variant 只能算 learning/recovery；transfer 只認下一個未見 variant 的第一次作答正確。</p></section><section class="quiz-list">'+groups.map(group=>{
        ...
        startedAt[q.id]=startedAt[q.id]||Date.now(); const status=m.fullyRetained?'R4':m.retained?'R'+m.retentionStage:m.due?'Review due':m.transfer?'Transfer pass':m.recovery?'Recovered':'Baseline/Transfer';
        return '<article class="quiz-card"><div class="quiz-head"><span class="tag">'+esc(q.module.tag)+' · '+esc(q.assessmentRole)+' · '+esc(q.variantId)+'</span><span class="evidence-level '+(m.retained?'level-4':m.transfer?'level-3':'level-1')+'">'+esc(status)+'</span></div><code>'+esc(q.competency)+'</code><h3>'+esc(q.prompt)+'</h3><label>作答信心<select data-confidence="'+esc(group.familyId)+'"><option value="0.5">50%</option><option value="0.7" selected>70%</option><option value="0.9">90%</option></select></label><div class="quiz-options">'+q.options.map(option=>'<button type="button" class="quiz-option" data-question="'+esc(q.id)+'" data-option="'+esc(option.id)+'">'+esc(option.text)+'</button>').join("")+'</div><div class="quiz-result" id="result-'+esc(group.familyId)+'" aria-live="polite">'+(feedback&&feedback.familyId===group.familyId?feedback.html:'')+'</div></article>';
      }).join("")+'</section>'+(numeric.length?'<section class="section-head"><h2>Numeric open-response</h2><p class="muted">不給選項，只接受數值、單位與容差。</p></section><section class="lab-grid">'+numeric.map(task=>'<article class="lab"><span class="tag">'+esc(task.moduleId)+'</span><h3>'+esc(task.prompt)+'</h3><div class="actions"><input class="search" style="max-width:180px" data-numeric-answer="'+esc(task.id)+'" inputmode="decimal" placeholder="數值"><input class="search" style="max-width:110px" data-numeric-unit="'+esc(task.id)+'" value="'+esc(task.unit)+'"><button type="button" class="button primary" data-numeric-submit="'+esc(task.id)+'">檢查</button></div><p data-numeric-result="'+esc(task.id)+'" aria-live="polite"></p></article>').join("")+'</section>':'')+'</main>';
      root.querySelectorAll("[data-question]").forEach(button=>button.addEventListener("click",()=>{
        const q=questions.find(x=>x.id===button.dataset.question)||groups.flatMap(g=>[Assessment.nextQuestion(g.questions,state.questions[g.familyId],Date.now())]).find(x=>x&&x.id===button.dataset.question); if(!q)return;
        const option=q.options.find(x=>x.id===button.dataset.option); if(!option)return;
        const stateNow=loadState(), confidence=root.querySelector('[data-confidence="'+CSS.escape(q.familyId)+'"]');
        Assessment.recordAttempt(stateNow,q,option,{at:now(),elapsedMs:Math.max(0,Date.now()-(startedAt[q.id]||Date.now())),confidence:confidence?Number(confidence.value):0.7,hintsUsed:0}); saveState(stateNow);
        const correct=q.options.find(x=>x.correct); feedback={familyId:q.familyId,html:option.correct?'<div class="quiz-explain"><b>正確</b><p>'+esc(option.feedback)+'</p></div>':'<div class="quiz-explain misconception"><b>可能迷思：'+esc(option.misconception||"推理鏈不完整")+'</b><p>'+esc(option.feedback)+'</p><p><strong>本 variant 正解：</strong>'+esc(correct.text)+'</p><p>這一題已不能再當 transfer 證據；系統會換未見 variant。</p></div>'}; render();
      }));
      if(Challenges) root.querySelectorAll("[data-numeric-submit]").forEach(button=>button.addEventListener("click",()=>{
        const id=button.dataset.numericSubmit, answer=root.querySelector('[data-numeric-answer="'+CSS.escape(id)+'"]').value, unit=root.querySelector('[data-numeric-unit="'+CSS.escape(id)+'"]').value;
        const previous=(loadState().openResponses[id]||[]).length, result=Challenges.evaluateNumeric(id,answer,unit); Evidence.recordOpenResponse(id,{...result,answer,enteredUnit:unit,firstAttempt:previous===0});
  ...
  function moduleNext(module,state) {
    ...
    const due=groups.some(group=>familyMetrics(group.familyId,state).due); if(due)return{label:"做 spaced review",href:"quiz.html?module="+module.id};
  ...
  function renderProgress(rootId) {
    ...
    document.getElementById("stateFile").onchange=async event=>{try{const file=event.target.files[0];if(!file)return;if(file.size>2_000_000)throw new Error("狀態檔過大");const payload=JSON.parse(await file.text());Evidence.merge(payload);location.reload();}catch(error){document.getElementById("stateMessage").textContent=error.message;}};
  }
  ...
  function renderReport(rootId) {
    ...
    const select=document.getElementById("labSelect"); select.innerHTML=allLabs.map(x=>'<option value="'+esc(x.lab.id)+'">'+esc(x.module.title+' — '+x.lab.title)+'</option>').join(""); const exact=allLabs.find(x=>x.lab.id===requested),legacy=allLabs.find(x=>x.lab.localId===requested); if(exact||legacy)select.value=(exact||legacy).lab.id;
    const fields=["observation","explanation","limitations","transfer","nextStep"], current=()=>allLabs.find(x=>x.lab.id===select.value)||allLabs[0], draft=()=>Object.fromEntries(fields.map(id=>[id,document.getElementById(id).value.trim()]));
    ...
    function persist(){const x=current();Evidence.setReport(x.lab.id,{...draft(),draft:true});preview();}
    function preview(){const x=current(),d=combinedDraft(),status=Evidence.predictionStatus(x.lab.id),events=Evidence.machineEvents(x.lab.id),verified=events.find(e=>e.verification&&e.verification.passed);document.getElementById("reportPreview").textContent='# '+x.module.title+' — '+x.lab.title+'\n\nPrediction integrity: '+(status.preRegistered?'preregistered':'post-hoc / not committed')+'\nMachine oracle: '+(verified?'PASS '+verified.verification.model.id+'@'+verified.verification.model.version:'not verified')+'\n\n'+Object.entries(d).map(([k,v])=>'## '+k+'\n'+(v||'- 尚未填寫')).join('\n\n');}
    document.getElementById("commitPrediction").onclick=()=>{try{const x=current();Evidence.commitPrediction(x.lab.id,{prediction:document.getElementById("prediction").value,parameters:document.getElementById("parameters").value,confidence:Number(document.getElementById("predictionConfidence").value)});refresh();}catch(error){document.getElementById("predictionStatus").textContent=error.message;}};
