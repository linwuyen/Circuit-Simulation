(()=>{
 ...
 let model=B.buckConfig(),control={kp:.3,ki:100,delayUs:10},previous=null,changed=[];
 function plot(id,rows,xlabel,ylabel,log=false,before=[]){const svg=$(id);svg.replaceChildren();if(!rows.length)return;const ns='http://www.w3.org/2000/svg';const node=(tag,attrs,text)=>{const n=document.createElementNS(ns,tag);for(const[k,v]of Object.entries(attrs))n.setAttribute(k,v);if(text)n.textContent=text;svg.append(n);return n;};const transform=x=>log?Math.log10(x):x,all=[...rows,...before],loX=Math.min(...all.map(r=>transform(r.x))),hiX=Math.max(...all.map(r=>transform(r.x))),loY=Math.min(0,...all.map(r=>r.y)),hiY=Math.max(1,...all.map(r=>r.y))*1.05;node('path',{d:'M65 20V215H775',stroke:'#a0b4c7',fill:'none'});if(before.length)node('polyline',{points:before.map(r=>`${65+(transform(r.x)-loX)/Math.max(1e-12,hiX-loX)*710},${215-(r.y-loY)/Math.max(1e-12,hiY-loY)*185}`).join(' '),fill:'none',stroke:'#aebbc9','stroke-width':2,'stroke-dasharray':'7 5'});node('polyline',{points:rows.map(r=>`${65+(transform(r.x)-loX)/Math.max(1e-12,hiX-loX)*710},${215-(r.y-loY)/Math.max(1e-12,hiY-loY)*185}`).join(' '),fill:'none',stroke:'#6cddbb','stroke-width':3});node('text',{x:5,y:20},ylabel);node('text',{x:520,y:252},xlabel);node('text',{x:5,y:44},hiY.toFixed(1));node('text',{x:5,y:212},loY.toFixed(1));node('text',{x:65,y:235},rows[0].x.toFixed(1));node('text',{x:695,y:235},rows.at(-1).x.toFixed(1));}
 function render(){const v=CircuitSharedCase.views(model,control),p=v.physical;
  ...
  plot('case-physical',p.rows.map(r=>({x:r.tUs,y:r.iL})),'時間（微秒）','電流 A',false,previous?.physical.rows.map(r=>({x:r.tUs,y:r.iL}))||[]);
  ...
  plot('case-transient',v.transient?v.transient.points.map(r=>({x:r.tS*1000,y:r.voutV})):[],'時間（毫秒）','電壓 V',false,previous?.transient?.points.map(r=>({x:r.tS*1000,y:r.voutV}))||[]);
  plot('case-frequency',v.frequency.map(r=>({x:r.frequencyHz,y:r.magnitudeDb})),'頻率 Hz（對數）','幅度 dB',true,previous?.frequency.map(r=>({x:r.frequencyHz,y:r.magnitudeDb}))||[]);
 ...
 function renderComparison(current){
  const body=$('case-comparison').querySelector('tbody');body.replaceChildren();
  $('case-change').textContent=previous?(changed.length?'本次改動：'+changed.join('、')+'。':'套用的設定與原先相同。'):'尚未改動。先改一個設定，這裡會自動保留改動前的數字。';
  const rows=[['輸出電壓','V',v=>v.physical.vout],['平均電流','A',v=>v.physical.avgI],['電流最高點','A',v=>v.physical.peakI],['命令生效時刻','微秒',v=>v.commitUs],['儀器還原的電壓','V',v=>v.sensing.reconstructedV],['恢復曲線最高電壓','V',v=>v.transient?Math.max(...v.transient.points.map(p=>p.voutV)):null],['恢復曲線最後電壓','V',v=>v.transient?.points.at(-1).voutV??null]];
  for(const[label,unit,get]of rows){const tr=document.createElement('tr'),a=previous?get(previous):null,b=get(current);[label,a===null?(previous?'此模型不適用':'尚無改動'):a.toFixed(3)+' '+unit,b===null?'此模型不適用':b.toFixed(3)+' '+unit,a===null||b===null?'—':(b-a>0?'+':'')+(b-a).toFixed(3)+' '+unit].forEach((text,i)=>{const n=document.createElement(i===0?'th':'td');if(i===0)n.scope='row';n.textContent=text;tr.append(n);});body.append(tr);}
  const note=document.createElement('tr'),cell=document.createElement('td');cell.colSpan=4;cell.textContent='控制波形可用性：'+(previous?(previous.modelValid?'先前可用':'先前停用')+' → ':'')+(current.modelValid?'現在可用':'現在停用，請看模型說明');note.append(cell);body.append(note);
 }

 function apply(){document.querySelectorAll('[data-case]').forEach(n=>n.value=model[n.dataset.case]);render();}
 document.querySelectorAll('[data-case],[data-control]').forEach(input=>input.addEventListener('change',()=>{try{if(!input.value.trim())throw new Error('請填寫完整數值');const nextModel=input.dataset.case?B.buckConfig({...model,[input.dataset.case]:Number(input.value)}):model;const nextControl=input.dataset.control?{...control,[input.dataset.control]:Number(input.value)}:control;CircuitSharedCase.views(nextModel,nextControl);previous=CircuitSharedCase.views(model,control);changed=[input.closest("label").childNodes[0].textContent.trim()];model=nextModel;control=nextControl;render();$('case-storage').textContent='設定已更新所有視圖；跨頁前請保存共用設定。';}catch(e){$('case-storage').textContent=e.message;input.value=input.dataset.case?model[input.dataset.case]:control[input.dataset.control];}}));
 ...
 $('case-load-saved').onclick=()=>{try{const incoming=U.validateScenario(U.read().scenarios?.['buck-steady-v1']).model;previous=CircuitSharedCase.views(model,control);changed=Object.keys(model).filter(k=>model[k]!==incoming[k]);model=incoming;apply();$('case-storage').textContent='已套用同模型設定；目前控制器參數保持本頁設定。';}catch(e){$('case-storage').textContent=e.message;}};
 render();
})();
