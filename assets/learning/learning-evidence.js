(function (root, factory) {
    ...
    if (v4 && v4.schema === SCHEMA) copyLegacyState(state, v4, "v4");
      ...
      Object.entries(v2.completed || {}).forEach(([id, value]) => {
        state.evidence[id] = {
          ...
          stage: value && value.evidence === "worksheet" ? "verified" : "practiced",
          strength: value && value.evidence === "worksheet" ? "C" : "none",
  ...
  function stageFromLevel(level) {
    return ["not-started","viewed","practiced","verified","retained"][Math.max(0, Math.min(4, Number(level) || 0))];
  }
  ...
  function strongestStrength(a, b) {
    return (STRENGTH[a] || 0) >= (STRENGTH[b] || 0) ? (a || "none") : (b || "none");
  }
  ...
  function recordEvidence(itemId, level, source, data, meta) {
    ...
    const sources = Array.from(new Set([...(current.sources || []), source || "unknown"]));
  ...
  function recordMachine(itemId, source, snapshot, verification) {
    ...
    if (!last || last.digest !== digest) {
      machine.push({ at: now(), source: source || "simulator", digest, ...record });
    ...
    state.evidence[itemId] = {
      ...
      firstMachineAt: current.firstMachineAt || now(),
      at: now()
    };
  ...
  function commitPrediction(itemId, prediction) {
    ...
    const revision = {
      ...
      preRegistered: !evidence.firstMachineAt || Date.parse(committedAt) <= Date.parse(evidence.firstMachineAt)
    };
  ...
  function getPrediction(itemId) {
    ...
    if (!record) return { revisions: [], active: null };
    return clone(record);
  }
  ...
  function predictionStatus(itemId) {
    ...
    const evidence = state.evidence[itemId] || {};
    const firstMachineAt = evidence.firstMachineAt || null;
    const preRegistered = !!(first && (!firstMachineAt || Date.parse(first.committedAt) <= Date.parse(firstMachineAt)));
    return { committed: !!first, first, active: record.active || null, firstMachineAt, preRegistered };
  }

  function reportRecord(value) {
    if (!value) return { revisions: [], active: null };
    if (Array.isArray(value.revisions)) return value;
    return { revisions: [{ id: "legacy", at: value.updatedAt || now(), ...value }], active: value };
  }
  ...
  function getReport(itemId) {
    const value = load().reports[itemId];
    const record = reportRecord(value);
    return clone(record.active || {});
  }

  function getReportHistory(itemId) {
    return clone(reportRecord(load().reports[itemId]).revisions);
  }
  ...
  function merge(payload) {
    ...
    Object.entries(payload.reports || {}).forEach(([id, incoming]) => {
      ...
      state.reports[id] = { revisions, active: revisions[revisions.length - 1] || null };
    });
