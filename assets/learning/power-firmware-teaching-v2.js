(function (global) {
  ...
  if (!Learning || typeof Learning.renderHome !== "function" || !Store || !Models) return;
  ...
  function mount(root) {
    ...
    insertEnhancements(root);
    bind(root);
    render(root);
    const unsubscribe = Store.subscribe(() => render(root));
    root.dataset.powerTeachingV2 = "mounted";
    global.CircuitPowerTeachingV2 = { render: () => render(root), destroy: unsubscribe };
  }
