(function(root,factory){const api=factory(root);if(typeof module==='object'&&module.exports)module.exports=api;root.CircuitUnifiedLearning=api;})(globalThis,function(root){
  ...
  function next(e={},flow={},at=Date.now()){
    ...
    const savedWorkspace=e.benchmark?.learningWorkspace;
    if(u.track!=='core'&&u.track!=='specialize'&&['experiment','topologyTransfer','topologyApplications'].some(key=>savedWorkspace?.[key]&&savedWorkspace[key].version!==1))return{id:'records',reason:'這份練習紀錄的版本目前無法讀取；請先查看與備份紀錄，再決定如何接續。'};
    const workspace=root.CircuitWorkspace||(typeof require==='function'?require('./workspace-core.js'):null);
    const ready=typeof require==='function'||(root.CircuitEngineeringCurriculum&&root.CircuitPlainCourse);
    const practice=e.benchmark?.learningWorkspace&&workspace&&ready?workspace.practiceRecords(e):[];
    const proven=id=>{const group=practice.find(g=>g.id===id);return !!group&&group.total>0&&group.completed===group.total;};
    const experimentDone=proven('experiment'),topologyDone=proven('topology'),applicationsDone=proven('applications');
    const applications=e.benchmark?.learningWorkspace?.topologyApplications;
    if(applications&&!applicationsDone&&u.track!=='core'&&u.track!=='specialize')return{id:'applications',reason:'接續原工具中的電路比較、觀察與原因判斷。'};
    const experiment=e.benchmark?.learningWorkspace?.experiment;
    if(experiment&&!experimentDone&&u.track!=='core'&&u.track!=='specialize')return {id:'experiment',reason:'接續同一台電路的連續實驗；原有課程成績保留。'};
    const topology=e.benchmark?.learningWorkspace?.topologyTransfer;
    if((experimentDone||topology)&&u.track!=='core'&&u.track!=='specialize'&&!topologyDone)return{id:'topology',reason:'沿用手動條件，辨認降壓與升壓能共用的觀念和不同限制。'};
    if(topologyDone&&!applicationsDone&&u.track!=='core'&&u.track!=='specialize')return{id:'applications',reason:'把同一種比較方法帶到 PFC、隔離電源、LLC 與逆變器。'};
    const topologyFamilies=root.CircuitAssessment?.topologyFamilyIds||[];
    if(applicationsDone&&topologyFamilies.length&&u.track!=='core'&&u.track!=='specialize'&&topologyFamilies.some(id=>!root.CircuitAssessment.metrics(e.questions?.[id],at).transfer))return{id:'topology-assessment',reason:'操作已完成，接著用不同條件檢查四種電路的觀念。'};
    const begun=Object.keys(flow.predictions||{}).length>0||Object.keys(flow.completed||{}).length>0;
    if(!experimentDone&&(u.track==='beginner'||(!begun&&u.track!=='core'&&u.track!=='specialize'))){
