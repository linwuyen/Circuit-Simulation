(function(root){
  ...
  function pathHtml(){return `<section id="powerFirmwarePath"><div class="section-head"><h2>Power Electronics Firmware Engineer Path</h2><p class="muted">不是追 module 數量；沿著 physical power → sensing → real-time control → protection → communication → system diagnosis 建立可遷移能力。</p></div><div class="lab-grid">${layers.map(l=>`<article class="lab"><div class="lesson-meta"><span class="tag blue">Layer ${esc(l.n)}</span></div><h3>${esc(l.title)}</h3><p>${esc(l.why)}</p><div class="actions">${l.items.map(i=>`<a class="button" href="${esc(moduleEntry(i[0]))}">${esc(i[1])}</a>`).join("")}</div></article>`).join("")}</div></section>`;}
  function ladderHtml(){const levels=[["L0","Recognize","辨認元件/訊號/狀態"],["L1","Calculate","算出量級與 unit"],["L2","Predict","操作前先預測方向"],["L3","Measure","選下一個最有資訊量的 measurement"],["L4","Diagnose","由 evidence 收斂 root cause"],["L5","Design","從 requirement 反推參數與 contract"],["L6","Integrate","串起 sensing/control/protection/communication"],["L7","Debug unknown","面對沒看過的 system 仍能逐層證偽"]];return `<section id="engineeringCapabilityLadder"><div class="section-head"><h2>Engineering Capability Ladder</h2><p class="muted">最高目標不是背公式，而是未知情境中的 first-attempt engineering judgment。</p></div><div class="fault-table">${levels.map(x=>`<article class="fault-row"><div><b>${x[0]}</b><span class="tag">${x[1]}</span></div><div><b>能力</b><p>${x[2]}</p></div></article>`).join("")}</div></section>`;}
  function fix(){
