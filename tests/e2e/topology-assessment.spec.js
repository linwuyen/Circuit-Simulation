...
test('home resumes unfinished topology assessment and returns to core after all four transfers',async({page})=>{
 await page.goto('/');await page.evaluate(()=>{localStorage.clear();const s=CircuitEvidence.load();s.benchmark.learningWorkspace={version:1,experiment:{version:1,completed:true,rows:Object.fromEntries(CircuitWorkspace.experimentLessons().map(l=>[l.id,{first:{correct:false},proof:{observation:true,reason:true},transferPassed:true}]))},topologyTransfer:{version:1,completed:true,protocol:'topology-workbench-v1',operated:true,rows:{prediction:{first:{correct:false}},observation:{passed:true},reason:{passed:true},return:{passed:true}}},topologyApplications:{version:1,completed:true,rows:Object.fromEntries(CircuitWorkspace.applicationLessons().map(l=>[l.id,{protocol:'topology-application-v1',first:{correct:false},operated:true,observationPassed:true,reasonPassed:true}]))}};CircuitEvidence.save(s);});
