...
test('all labs expose explicit verification contracts and honest grade ceilings', async ({ page }) => {
  ...
  await expect(page.locator('#mainContent')).toContainText('50/50 labs classified');
  await expect(page.locator('#mainContent')).toContainText('16/16 modules have an A path');
...
test('progress reports complete contract coverage without pretending all labs have ground truth', async ({ page }) => {
  ...
  await expect(page.locator('#labContractMatrix')).toContainText('50/50 classified');
  ...
  await expect(moduleCards).toHaveCount(16);
});
