...
test('first lesson guides an incorrect guess through observation, hints, transfer and reload',async({page})=>{
  ...
  await expect(page.locator('#experiment-control')).toBeDisabled();await expect(page.locator('#transfer-section')).toBeHidden();
  ...
  await page.locator('#transfer-value').fill('12');await page.locator('#transfer-submit').click();await expect(page.locator('#next-lesson')).toBeDisabled();
  ...
  await page.reload();await expect(page.locator('#progress')).toContainText('1 / 5');await expect(page.locator('input[name=prediction][value=down]')).toBeChecked();
...
test('playback pauses, stepping moves cursor, mobile layout fits and glossary is keyboard accessible',async({page})=>{
  const before=await page.locator('#cursor').getAttribute('x1');await page.locator('#step').click();expect(await page.locator('#cursor').getAttribute('x1')).not.toBe(before);
  ...
  const summary=page.locator('#glossary summary').first();await summary.focus();await page.keyboard.press('Enter');await expect(page.locator('#glossary details').first()).toHaveAttribute('open','');
