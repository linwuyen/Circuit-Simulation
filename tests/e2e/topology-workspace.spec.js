...
test('shared comparison gates observations, keeps wrong first attempts and resumes after refresh',async({page})=>{
 ...
 await expect(page.locator('#circuit')).toHaveAccessibleName('降壓與升壓的能量路徑示意');await expect(page.locator('#ws-plot')).toBeHidden();await expect(page.locator('#ws-title')).toContainText('降壓和升壓');await expect(page.locator('#ws-voltage')).toHaveText('先預測');
