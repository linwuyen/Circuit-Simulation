...
test("core8 v2 keeps eight questions while exposing two families per competency", () => {
  assert.equal(Families.VERSION, 2);
  assert.deepEqual(Families.COMPETENCIES, Benchmark.CORE8_COMPETENCIES);
  for (const competency of Families.COMPETENCIES) {
    assert.equal(Families.FAMILY_CATALOG[competency].length, 2);
    assert.equal(new Set(Families.FAMILY_CATALOG[competency].map(row => row.id)).size, 2);
  }
  const cases = Families.generateBenchmarkSet({ seed:20260821, phase:"pre" });
...
test("family forms vary across seeds without changing phase burden", () => {
  ...
  for (const seed of [1,2,3,4,5,6,7,8]) {
    const cases = Families.generateBenchmarkSet({ seed, phase:"post" });
    assert.equal(cases.length, 8);
...
test("v2 PRE/POST/R1-R4 remain content-disjoint for one learner seed", () => {
  const phases = ["pre","post","r1","r2","r3","r4"];
  const all = phases.flatMap(phase => Families.generateBenchmarkSet({ seed:20260821, phase }));
  assert.equal(all.length, 48);
  assert.equal(new Set(fingerprints(all)).size, 48);
  assert.equal(new Set(all.map(item => item.id)).size, 48);
});

test("same seed and phase is deterministic, and contract fingerprint is seed-independent", () => {
  const a = Families.generateBenchmarkSet({ seed:77, phase:"pre" });
  const b = Families.generateBenchmarkSet({ seed:77, phase:"pre" });
  assert.deepEqual(a, b);
  assert.match(Families.contractFingerprint(), /^core8-families-v2-[0-9a-f]{8}$/);
});

test("timing v2 preserves strict exactly-on-load miss semantics", () => {
  let exact = null;
  for (let seed = 0; seed < 32 && !exact; seed += 1) {
    exact = Families.generateBenchmarkSet({ seed, phase:"pre" }).find(item => item.competency === "timing" && item.familyId === "timing.shadow-load-deadline" && item.parameters.ratio === 1);
  }
